...
const CalculateTotalValueUseCase = class {
  ...
  async execute(request) {
    try {
      ...
      if (!item) {
        return { success: false, error: new Error(`Item with id ${request.itemId} not found`) };
      }
...
async function main() {
  ...
  if (actualTotal !== expectedTotal) {
    console.error(`❌ Calculation error: expected ${expectedTotal}, got ${actualTotal}`);
    process.exit(1);
  }
  ...
  console.log('   ✓ Domain layer: Pure business logic (Money, Quantity value objects)');
  console.log('   ✓ Application layer: Use case orchestration');
  console.log('   ✓ Infrastructure layer: Repository implementation (in-memory)');
  console.log('   ✓ Dependency inversion: Use case depends on port, not concrete implementation');
  console.log('   ✓ Testability: Can swap in-memory repo with real database without changing use case\n');
