...
export async function register() {
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    ...
    if (!validation.success) {
      ...
      if (validation.environment === 'production' || validation.environment === 'staging') {
        throw new Error('Environment validation failed in production/staging. Cannot start server.');
      }
      
      // In preview/local, log error but allow startup for development
      console.warn('⚠️  Continuing with invalid environment in preview/local mode.');
