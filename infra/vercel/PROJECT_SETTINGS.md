...
> UTA-11: PR previews no longer share one `DATABASE_URL`. Each PR gets an
> isolated Neon child branch of Staging (`preview/pr-<n>`) whose URL is wired
> as a **branch-scoped** Vercel env var (`target: preview`,
> `gitBranch: <PR head branch>`) by `infra/neon/preview-setup.mjs` and removed
> on PR close. Full flow: [PREVIEW_WIRING.md](./PREVIEW_WIRING.md).

| Key                     | Value Example                                                        | Environments                  | Notes                           |
| ----------------------- | -------------------------------------------------------------------- | ----------------------------- | ------------------------------- |
| `DATABASE_URL`          | `postgresql://user:pass@<per-pr-branch-endpoint>.neon.tech/tokoboss` | Preview, branch-scoped per PR | Set by automation, never shared |
